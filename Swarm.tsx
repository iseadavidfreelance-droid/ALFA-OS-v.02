...
export default function Swarm({ assets }: SwarmProps) {
  ...
  const fetchPins = async () => {
    setLoading(true);
    // Solo traemos pines que están asignados a un activo (No huérfanos)
  ...
  const handlePurge = async () => {
    ...
    setPurging(true);
    try {
      // Desvincular pines muertos (asset_id = null)
      ...
      // Recargar datos para reflejar la limpieza
      await fetchPins();
    } catch (err) {
  ...
  return (
    <div className="w-full h-full overflow-y-auto bg-void-black p-8 space-y-12 custom-scrollbar font-mono">
      ...
      <style>{`
        ...
        .orbit-satellite {
          /* Counter-rotate to keep satellite upright relative to screen if needed, 
             but for dots it doesn't matter. 
             If we wanted icons to face up: animation: orbit 20s linear reverse infinite; 
          */
        }
      `}</style>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-12 pb-12">
          {assets.map(asset => {
            ...
            // Si es un planeta fantasma (sin satélites), lo mostramos opaco
            if (!hasSatellites) {
              return (
            ...
            return (
              <div key={asset.id} className="relative w-full aspect-square flex items-center justify-center">
                {/* ORBIT TRACK (Static) */}
                ...
                {/* ROTATING ORBIT RING */}
                <div className="orbit-ring absolute w-[80%] h-[80%] rounded-full pointer-events-none">
                  {satellites.map((pin, i) => {
                    ...
                    return (
                      <div
                        key={pin.id}
                        className="absolute top-1/2 left-1/2 w-3 h-3 -ml-1.5 -mt-1.5 rounded-full shadow-sm"
                        style={{
                          transform: `rotate(${angle}deg) translate(500%) rotate(-${angle}deg)`, // Translate 500% moves it to the edge of the container roughly
                          backgroundColor: isDead ? '#ef4444' : '#00ff41',
                          boxShadow: isDead ? '0 0 5px #ef4444' : '0 0 5px #00ff41'
                        }}
                        title={`Pin: ${pin.id} | Clicks: ${pin.last_stats?.outbound_clicks}`}
                      ></div>
                    );
                  })}
                </div>
